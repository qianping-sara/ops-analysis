...
```bash
# 近 6 周每周 Top 主题聚合（JSON 周计数）
npm run topic:daily -- drift --weeks 6

# 新兴：近 7 天有、前 21 天无（字面 intent_theme）
npm run topic:daily -- emerging --recent 7 --baseline 28 --min-recent 2

# 消退：前 21 天有、近 7 天无
npm run topic:daily -- fading --recent 7 --baseline 28 --min-recent 2
```
